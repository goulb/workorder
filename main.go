...
func main() {
	...
	mux.HandleFunc("/orders/print", printOrders)
	...
	mux.HandleFunc("/orders/unlock", privilegeHandle(unlockOrder, data.CanEditAll))
	mux.HandleFunc("/orders/lock", privilegeHandle(lockOrder, data.CanEdit))
	...
	mux.HandleFunc("/workitems/delete", privilegeHandle(deleteWorkitem, data.CanEdit))
